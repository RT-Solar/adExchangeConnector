...
namespace Org.IdentityConnectors.Exchange
{
    abstract class SimplePowerShellObjectHandler : ObjectClassHandler
    {
        ...
        public void Create(CreateOpContext context)
        {
            ...
            try {
                ...
                if (nameAttribute == null || nameAttribute.Value == null || nameAttribute.Value.Count() != 1) {
                    LOG.Info("ProxyAddressExistsException reported; but no single-valued NAME attribute present -- reporting as is");
                    throw new ConnectorException(e.Message, e);
                }
                String name = (String) nameAttribute.Value[0];
                LOG.Info("ProxyAddressExistsException reported; trying to see if object named " + name + " exists");
                ...
                if (objects == null || objects.Count == 0) {
                    LOG.Info("...it does not -- reporting as is");
                    throw new ConnectorException(e.Message, e);
                } else {
                    LOG.Info("...it exists -- reporting as AlreadyExistsException");
        ...
        public void ExecuteQuery(ExecuteQueryContext context)
        {
            ...
            LOG.Trace("SimplePowerShellObjectHandler: Executing query: query={0}", context.Query);
            ...
            try {
                ...
                LOG.Trace("SimplePowerShellObjectHandler: Executing query: got 'ObjectNotFound' exception ({0}), assuming suitable objects do not exist", e);
                return;
            }
            LOG.Trace("SimplePowerShellObjectHandler: Executing query: got {0} objects", objects.Count);
