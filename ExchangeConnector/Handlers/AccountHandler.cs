    ...
    class AccountHandler : ObjectClassHandler {
        ...
        public void Create(CreateOpContext context) {
            ...
            if (policyEnabledAttribute != null &&
                ...
                LOG.Trace("Added empty EmailAddresses attribute because address policy use is disabled and no addresses were provided");
            } else {
            ...
            try {
                ...
                LOG.Warn("Rolling back AD create for UID: " + uid.GetUidValue());

                // rollback AD create
                try {
                    adconn.Delete(context.ObjectClass, uid, context.Options);
                } catch {
                    LOG.Warn("Not able to rollback AD create for UID: " + uid.GetUidValue());
        ...
        private ICollection<ConnectorAttribute> DeduplicateEmailAddresses(CreateUpdateOpContext context, ICollection<ConnectorAttribute> attributes)
        {
                ...
                if (newPrimary != null)
                {
                    foreach (string address in new List<string>(valuesToDeduplicate))      // to eliminate concurrent access
                    {
                        if (address.StartsWith("SMTP:") && !address.Equals(newPrimary))
                        {
                            ...
                            LOG.Info("Changing duplicate primary-candidate address {0} to {1}", address, replacement);
            ...
            foreach (object v in valuesToDeduplicate) 
            {
                ...
                if (values.ContainsKey(normalized)) 
                {
                    ...
                    {
                        LOG.Info("Removing redundant address {0}, keeping {1}", existing, address);
                        values[normalized] = address;
                    }
                    else 
                    {
                        LOG.Info("Removing redundant address {0}, keeping {1}", address, existing);
                    }
                } 
                else 
                {
        ...
        public void ExecuteQuery(ExecuteQueryContext context)
        {
            ...
            ResultsHandler filter = new SearchResultsHandler()
            {
                Handle = cobject =>
                {
                    LOG.Trace("Object returned from AD connector: {0}", CommonUtils.DumpConnectorAttributes(cobject.GetAttributes()));
                    ...
                    LOG.Trace("Object as passed from Exchange connector: {0}", CommonUtils.DumpConnectorAttributes(filtered.GetAttributes()));
