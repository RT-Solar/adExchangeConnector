    ...
    internal class CustomAttributeHandlers
    {
            ...
            ConnectorAttribute attribute)
        {
            if (oclass.Equals(ObjectClass.ACCOUNT))
            {
                ...
                LOG.Debug("UpdateDeFromCa_OpAtt_Groups: user = {0}, oldValues = {1}, newValues = {2}, updateType = {3}",
                ...
                LOG.Debug("UpdateDeFromCa_OpAtt_Groups: user = {0}, groupsToAdd = {1}, groupsToRemove = {2}",
                ...
                foreach (Object obj in groupsToRemove)
                {
                    ...
                    LOG.Debug("UpdateDeFromCa_OpAtt_Groups: Group: {0}, members before = {1}, removing {2}",
                    ...
                    {
                        LOG.Warn("{0} is NOT a member of {1}, no remove operation is executed", distinguishedName, groupPath);
                    }
                    groupDe.Dispose();
                }
                ...
                foreach (Object obj in groupsToAdd)
                {
                    ...
                    DirectoryEntry groupDe = new DirectoryEntry(groupPath,
                        _configuration.DirectoryAdminName, _configuration.DirectoryAdminPassword);
                    String distinguishedName = ActiveDirectoryUtils.GetDnFromPath(directoryEntry.Path);
                    LOG.Debug("UpdateDeFromCa_OpAtt_Groups: Group: {0}, members before = {1}, adding {2}",
                        groupPath, DumpPVC(groupDe.Properties[ActiveDirectoryConnector.ATT_MEMBER]), distinguishedName);
                    if (!groupDe.Properties[ActiveDirectoryConnector.ATT_MEMBER].Contains(distinguishedName))
                    {
                    ...
                    {
                        LOG.Warn("{0} is already a member of {1}, no add operation is executed", distinguishedName, groupPath);
                    }
                    groupDe.Dispose();
                }
            }
            ...
            ConnectorAttribute attribute)
        {
            if (ActiveDirectoryConnector.groupObjectClass.Equals(oclass))
            {
                ...
                while (true) {
                    ...
                    if (directoryEntry.Properties[attribute.Name].Count < rangeDelta || type.Equals(UpdateType.ADD))
                    {
                        //No need to refresh, because it last partition or add operation
                        break;
                    }
                    directoryEntry.CommitChanges();
                    range += rangeDelta;                 
                    directoryEntry.RefreshCache(new[] { $"member;range={range}-*" });                    
                }                                                                
                    ...
                    foreach (Object valueObject in attribute.Value)
                    {
                        ...
                        if (caseInsensitive && valueObject is string)
                        {
                            ...
                            foreach (object existing in pvc)
                            {
                                ...
                                {
                                    LOG.Debug("existing value is not a string, it is: {0}", existing.GetType());
                        ...
                        if (!foundAndRemoved)
                        {
                            LOG.Warn("Can't find value {0} from {1} in this partition. Current values = {2}", valueObject, attribute.Name, DumpPVC(pvc));                            
                        }                        
                    }                
                }
            ...
            ObjectClass oclass, string attributeName, DS.SearchResult searchResult, DirectoryEntry entry)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < pvc.Count; i++)
                {
                    ...
                    {
                        LOG.Debug("Unsupported attribute type ... calling ToString (Name: \'{0}\'({1}) Type: \'{2}\' String Value: \'{3}\'",
