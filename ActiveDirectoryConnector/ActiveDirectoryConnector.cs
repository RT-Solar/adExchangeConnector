...
namespace Org.IdentityConnectors.ActiveDirectory
{
    ...
    public class ActiveDirectoryConnector : CreateOp, Connector, SchemaOp, DeleteOp,
        ...
        AuthenticateOp, ResolveUsernameOp, PoolableConnector
    {
            ...
            ICollection<ConnectorAttribute> attributes, OperationOptions options)
        {
            ...
            watch.Start();
            LOG.Info("AD.Create method starting; oclass: {0}, attributes: {1}", 
            ...
            LOG.Info("Name attribute = {0}", nameAttribute.GetDetails());
            ...
            LOG.Debug("LdapContainerPath = {0}, LdapEntryPath = {1}", ldapContainerPath, ldapEntryPath);
            ...
            try
            {
                ...
                ICollection<ConnectorAttribute> realAttributes = new HashSet<ConnectorAttribute>(attributes);
                ConnectorAttribute userAccountControlAttribute = null;
                ConnectorAttribute member = null;
                ConnectorAttribute memberOf = null;
                if (_configuration.OnAccountCreateTolerateGroups)
                {
                    member = ConnectorAttributeUtil.Find(ATT_MEMBER, realAttributes);
                    if (member != null)
                        realAttributes.Remove(member);
                    memberOf = ConnectorAttributeUtil.Find(ATT_MEMBEROF, realAttributes);
                    if (memberOf != null)
                        realAttributes.Remove(memberOf);
                }
                foreach (ConnectorAttribute attr in realAttributes)
                {
                  if (attr.Name.Equals(ATT_USER_ACOUNT_CONTROL))
                    {
                      userAccountControlAttribute = attr;
                      realAttributes.Remove(attr);
                      break;
                    }
                }
                // default to creating users enabled
                if ((ObjectClass.ACCOUNT.Equals(oclass)) &&
                    (ConnectorAttributeUtil.Find(OperationalAttributes.ENABLE_NAME, realAttributes) == null))
                {
                    ICollection<ConnectorAttribute> temp = new HashSet<ConnectorAttribute>(realAttributes);
                ...
                {
                    _utils.UpdateADObject(oclass, newDe, realAttributes, UpdateType.REPLACE, _configuration);
                }

                if (userAccountControlAttribute != null)
                {
                    ICollection<ConnectorAttribute> temp = new HashSet<ConnectorAttribute>();
                    temp.Add(userAccountControlAttribute);
                    _utils.UpdateADObject(oclass, newDe, temp, UpdateType.REPLACE, _configuration);
                }
                if (_configuration.OnAccountCreateTolerateGroups)
                {
                    ...
                    catch (Exception tolerateGroupsException)
                    {
                        LOG.Info("Caught general exception while member and memberof create: " + tolerateGroupsException.Message);
                    }
                }
                ...
                if (guidValue != null)
                {
                    ...
                    LOG.Debug("Created object with uid {0}", guidString);
                    uid = new Uid(guidString);
                }
                else
                {
                    LOG.Error("Unable to find uid attribute for newly created object");
                }
            }
            catch (DirectoryServicesCOMException exception)
            {
                ...
                // the case of error
                LOG.Error(exception, "Caught COM exception");
                if (created)
                {
            ...
            catch (Exception exception)
            {
                ...
                LOG.Error(exception, "Caught general exception");
                if (created)
                {
                    // In the case of an exception, make sure we
                    // don't leave any partial objects around
            ...
            LOG.Info("AD.Create returning UID: {0}, finishing in {1} ms", uid.GetUidValue(), watch.ElapsedMilliseconds);
            return uid;
        }
        ...
        {
            LOG.Info("AD.Init method starting");
            ...
            LOG.Debug("Search: Getting root node for search");
            ...
            LOG.Info("AD.Init method finishing");
        }
        ...
        {
            LOG.Info("AD.Schema method starting");
            if (_schema != null)
            {
                LOG.Info("AD.Schema method exiting, returning cached schema");
                return _schema;
            }
            else
            {
                ...
                _attributesReturnedByDefault = SchemaUtils.GetAttributesReturnedByDefault(
                                GetSupportedObjectClasses,
                                GetObjectClassInfo);
                LOG.Info("AD.Schema method exiting, returning freshly computed schema");
                return _schema;
            }
        }
        ...
        {
            LOG.Info("ExecuteQuery starting, query = {0}", query);
                ...
                if ((query == null) || (query.Length == 0))
                {
                    if ((options != null) && (options.Options != null))
                    {
                        ...
                        if ((oldStyleQuery != null) && (oldStyleQuery is string))
                        {
                            ...
                            LOG.Warn(_configuration.ConnectorMessages.Format(
            ...
            string searchRoot, SearchScope searchScope, ICollection<string> attributesToReturn)
        {
            ...
            LOG.Debug("AD.ExecuteQueryInternal: modifying query; attributesToReturn = {0}, pageSize = {1}", 
            ...
            else
            {
                ...
                LOG.Warn("Setting search string to \'{0}\'", query);
            }
            ...
            try
            {
                ...
                if (pagedSearch)
                {
                    ...
                    {
                        LOG.Warn("PagedResultsCookie is not supported, the provided value of '{0}' will be ignored.", options.PagedResultsCookie);
                    }
                    ...
                    LOG.Debug("Search: creating VLV: BeforeCount={0}, AfterCount={1}, Offset={2}",
                    ...
                    if (sortOption == null)
                    {
                        ...
                        LOG.Debug("No sort option specified, but VLV requires one. Default one will be used.");
                    }
                }
                else
                {
                ...
                if (sortOption != null)
                {
                    ...
                    LOG.Debug("Sort option: {0} / {1}", sortOption.PropertyName, sortOption.Direction);
                }

                LOG.Debug("Search: Performing query");
                ...
                try
                {
					...
					string elapsedTime = String.Format("{0:00}:{1:00}:{2:00}.{3:000}",
	        			...
	        			ts.Milliseconds);
   					LOG.Debug("searcher.FindAll took {0}", elapsedTime);
                    ...
                    foreach (DS.SearchResult result in resultSet)
                    {
                        ...
                        {
                            LOG.Debug("Processing search results was stopped on the client request");
                            break;
                        }
                    }
                    ...
                    if (pagedSearch)
                    {
                        ...
                        {
                            LOG.Warn(e, "Couldn't get VirtualListView from the DirectorySearcher instance");
                        }
                        if (vlv != null)
                        {
                            totalCount = vlv.ApproximateTotal;
                            LOG.Debug("VLV returned = {0}; BeforeCount={1}, AfterCount={2}, Offset={3}, ApproxTotal={4}, DirVLVContext={5}, Target={6}, TargetPerc={7}", vlv, 
                ...
                finally
                {
                    ...
                    LOG.Debug("Search: found {0} results, took {1}", count, elapsedTime);
        ...
        private bool buildConnectorObject(DS.SearchResult result, ObjectClass oclass, bool useGlobalCatalog, string searchRoot, ICollection<string> attributesToReturn, ResultsHandler handler)
        {
            ...
            try
            {
                LOG.Debug("Found object {0}", result.Path);
                ...
                if (isDeleted.Equals(false))
                {
                    ...
                    if (useGlobalCatalog)
                    {
                        ...
                        LOG.Debug("after dcSearcher.FindOne: T={0} ms", stopWatch.ElapsedMilliseconds);
                        ...
                        LOG.Debug("after dcSearchRoot.Dispose: T={0} ms", stopWatch.ElapsedMilliseconds);
                    }
            ...
            catch (DirectoryServicesCOMException e)
            {
                ...
                LOG.Error(e, "Error in creating ConnectorObject from DirectoryEntry. It may have been deleted during search");
            }
            catch (Exception e)
            {
                // In that case, of any error, try to continue
                LOG.Error(e, "Error in creating ConnectorObject from DirectoryEntry.");
            }
			stopWatch.Stop();
            return rv;
        }
        ...
        public virtual void Test()
        {
            ...
            {
                LOG.Error(dscex, "Failed to determine whether the Container '{0}' exists.", _configuration.Container);
            ...
            ICollection<ConnectorAttribute> attributes, OperationOptions options)
        {
            ...
            LOG.Info("AD.Update method; type = {0}, oclass = {1}, attributes: {2}", type, oclass, CommonUtils.DumpConnectorAttributes(attributes));

            if(type == UpdateType.ADD)
            {
                plugins.OnBeforeAttributesAdd(oclass, attributes, options, _configuration);
            }
            else if(type == UpdateType.DELETE)
            {
                plugins.OnBeforeAttributesDelete(oclass, attributes, options, _configuration);
            }
            else if(type == UpdateType.REPLACE)
            {
                plugins.OnBeforeAttributesReplace(oclass, attributes, options, _configuration);
            }
            ...
            {
                // nothing besides UID
                LOG.Info("AD.Update method finishing - nothing to do");
                return updatedUid;
            }
            ...
            {
                LOG.Error(e, "Got exception when updating");
                throw;
            }
            finally
            {
            ...
            LOG.Info("AD.Update method finishing in {0} ms", watch.ElapsedMilliseconds);
            return updatedUid;
        }
        ...
        {
            LOG.Info("AD.Delete; uid = {0}", uid != null ? uid.GetUidValue() : "(null)");
            ...
            LOG.Info("AD.Delete method finishing");
        }
        ...
        public class SyncResults
        {
            ...
            internal long LastSecurityEventId { get; set; }
                ...
                get
                {
                    return new ResultsHandler()
                    {
                        Handle = obj =>
                        {
                            ...
                            {
                                if(_adSyncToken.LastModifiedUsn < tokenUsnValue)
                                    _adSyncToken.LastModifiedUsn = tokenUsnValue;
                            }
                            if (_adSyncToken.LastSecurityEventId < LastSecurityEventId)
                            {
                                _adSyncToken.LastSecurityEventId = LastSecurityEventId;
                            }
        ...
        {
            LOG.Debug("Executing SyncInternal");
            ...
            if(ObjectClass.ACCOUNT.Equals(objClass) && _configuration.DiscoverEventLog) 
            {
                LOG.Debug("Discovering event log");

                // find affected group subjects
                try
                {
                    long lastEventId = SecurityLogReader.FetchLatestEventId(_configuration.EventLogComputerName, _configuration.EventLogDomainName,
                    _configuration.EventLogUserName, _configuration.EventLogUserPassword).GetValueOrDefault(0);
                    LOG.Debug("Fetched last event id: {0}", lastEventId);
                    syncResults.LastSecurityEventId = lastEventId;

                    HashSet<String> affectedSubjects = SecurityLogReader.FetchGroupsAffectedSubjects(_configuration.EventLogComputerName, _configuration.EventLogDomainName,
                        _configuration.EventLogUserName, _configuration.EventLogUserPassword, adSyncToken.LastSecurityEventId);
                    LOG.Debug("affectedSubjects size = ", affectedSubjects.Count);

                    foreach (String affectedSubjectDn in affectedSubjects)
                    {
                        LOG.Debug("Processing affected subject dn: {0}", affectedSubjectDn);
                        String dnQuery = "(distinguishedName=" + affectedSubjectDn + ")";
                        ExecuteQueryInternal(objClass, dnQuery, syncResults.SyncHandler, builder.Build(),
                            true, new SortOption(ATT_USN_CHANGED, SortDirection.Ascending),
                            serverName, UseGlobalCatalog(), deleteObjectsSearchRoot, SearchScope.Subtree, attributesToReturn);
                    }
                }
                ...
                {
                    LOG.Error(e, "Exception while discovering event log");
                }
            }
        }
        ...
        public SyncToken GetLatestSyncToken(ObjectClass objectClass)
        {
            ...
            long lastSecurityEventId = 0;
            if(_configuration.DiscoverEventLog)
            {
                lastSecurityEventId = SecurityLogReader.FetchLatestEventId(_configuration.EventLogComputerName, _configuration.EventLogDomainName,
                    _configuration.EventLogUserName, _configuration.EventLogUserPassword).GetValueOrDefault(0);
            }
            ...
            token.LastSecurityEventId = lastSecurityEventId;
        ...
        string GetSyncServerName()
        {
            ...
            {
                LOG.Warn("No server was configured for synchronization, so picking one. You should configure a server for best performance.");
        ...
        #region ResolveUsernameOp Members
