...
namespace Org.IdentityConnectors.ActiveDirectory
{
    public class ActiveDirectorySyncToken
    {
        ...
        public ActiveDirectorySyncToken(String tokenValue, string configServerName, bool configUseGlobalCatalog)
        {
            ...
            {
                throw new ConnectorException("Unable to parse sync token: " + tokenValue);
            }

            LastSecurityEventId = long.Parse(tokenParts[4]);
        ...
        {
            return new SyncToken(String.Format("{0}|{1}|{2}|{3}|{4}",
                LastModifiedUsn, LastDeleteUsn, UseGlobalCatalog, SyncServer, LastSecurityEventId));
        }
    }
}
