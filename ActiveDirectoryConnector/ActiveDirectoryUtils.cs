...
namespace Org.IdentityConnectors.ActiveDirectory
{
    ...
    public class ActiveDirectoryUtils
    {
        ...
        /// <returns></returns>
        public static String GetLDAPPath(string serverName, string path)
        {
        ...
        */
            ...
            ActiveDirectoryConfiguration config)
        {
            ...
            {
                LOG.Debug("Renaming {0} to {1}", oldName, newName);
            ...
            if (nameChanged)
            {
                ...
                LOG.Debug("Renaming {0} to a temporary name of {1}", oldName, temporaryName);
            ...
            try
            {
                ...
                LOG.Debug("Moving from {0} to {1} ({2})", oldContainer, newContainer, newContainerLdapPath);
            ...
            {
                LOG.Error(e, "Exception caught when moving");
                if (nameChanged)
                {
                    LOG.Debug("Renaming back from temporary name of {0} to {1}", temporaryName, oldName);
            ...
            if (nameChanged)
            {
                LOG.Debug("Renaming from temporary name of {0} to a new name of {1}", temporaryName, newName);
                directoryEntry.Rename(newName);
                LOG.Debug("Rename OK");
            }
        }
        ...
        public static String EscapeDnFilter(String ldapPath)
        {
            String 
            escaped = ldapPath.Replace("(", "\\28");
        ...
        internal ActiveDirectorySchema GetADSchema()
        {
            ...
            {
                // get the active directory schema
                LOG.Debug("Trying to lookup Domain controller for domain {0}", _configuration.DomainName);
                ...
                LOG.Debug("Found Domain controller named {0} with ipAddress {1} for domain {2}",
                    dc.Name, dc.IPAddress, _configuration.DomainName);
                forest = dc.Forest;
                LOG.Debug("Found forest");
            }
            else
            {
                DirectoryContext context = new DirectoryContext(
            ...
            LOG.Debug("Getting schema from AD");
            ActiveDirectorySchema ADSchema = forest.Schema;
            LOG.Debug("Got schema from AD");
        ...
        {
            var result = false;
            var pathName = getADSPathname(null, null, dn);
            try
            {
                if (pathName != null)
                {
        ...
        {
            LOG.Info("ErrorCode = {0}, ExtendedError = {1}, ExtendedErrorMessage = {2}",
        ...
        {
            LOG.Info("ErrorCode = {0}", originalException.ErrorCode);
