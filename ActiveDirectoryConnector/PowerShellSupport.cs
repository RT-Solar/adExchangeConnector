...
namespace Org.IdentityConnectors.ActiveDirectory
{
	...
	public sealed class PowerShellSupport : IPowerShellSupport, IDisposable {
		...
		public void Test()
		{
			...
			LOG.Trace(MethodName + ":entry", ClassName);
            ...
            LOG.Trace(MethodName + ":exit", ClassName);
		}
        ...
        public ICollection<PSObject> InvokePipeline(Collection<Command> commands) {
            ...
            LOG.Trace("InvokePipeline: Runspace acquired: {0} ms from start", watch.ElapsedMilliseconds);
            ...
            try {
                ...
                LOG.Trace("InvokePipeline: Method finishing: {0} ms from start", watch.ElapsedMilliseconds);
            }
        }
        ...
        public void ProcessRunspaceException(Runspace runspace, int attempt, InvalidRunspaceStateException e) {
            ...
            if (info != null) {
                LOG.Trace("Runspace is in wrong state. Exception: {0}, State: {1}, Reason: {2}, Attempt number: {3}",
                    e, info.State, info.Reason, attempt);
            } else {
                LOG.Warn("Runspace is in wrong state. Exception: {0}, Attempt number: {1}", e, attempt);
            }
            if (attempt == MAX_ATTEMPTS) {
                LOG.Warn("Maximum number of attempts achieved, signalling the exception");
        ...
        private ICollection<PSObject> InvokePipelineInternal(Runspace runspace, Collection<Command> commands) {
			...
			foreach (Command item in commands) {
				LOG.Trace("Item = {0}", item);
				foreach (CommandParameter cp in item.Parameters) {
					LOG.Trace(" - parameter {0} = {1}", cp.Name, cp.Value);
				}
				pipe.Commands.Add(item);
			}
			LOG.Trace("Executing pipe: {0}", pipe);

			// run the pipeline if we have something to execute
			results = pipe.Invoke();
            LOG.Trace("InvokePipelineInternal: Pipeline.Invoke returned: {0} ms from start of this method", watch.ElapsedMilliseconds);
            ...
            LOG.Trace("InvokePipelineInternal: Method finishing: {0} ms from start of this method", watch.ElapsedMilliseconds);
            LOG.Trace(MethodName + ":exit", ClassName);
			return results;
		}
        ...
        public ICollection<PSObject> InvokeScript(string script, ICollection<CommandParameter> parameters) {
            ...
            watch.Start();
            Runspace runspace = _runSpacePool.acquireRunspace();
            LOG.Trace("InvokeScript: Runspace acquired: {0} ms from start", watch.ElapsedMilliseconds);
            ...
            try {
                ...
                LOG.Trace("InvokeScript: Method finishing: {0} ms from start", watch.ElapsedMilliseconds);
            }
        }
        ...
        public ICollection<PSObject> InvokeScriptInternal(Runspace runspace, string script, ICollection<CommandParameter> parameters) {
            ...
            using (PowerShell powerShellInstance = PowerShell.Create()) {
                LOG.Trace("InvokeScriptInternal: Powershell instance created: {0} ms from start", watch.ElapsedMilliseconds);
                ...
                LOG.Trace("InvokeScriptInternal: Invoke started: {0} ms from start", watch.ElapsedMilliseconds); 
                results = powerShellInstance.Invoke();
                LOG.Trace("InvokeScriptInternal: Invoke returned: {0} ms from start", watch.ElapsedMilliseconds); 
                ...
                LOG.Trace(MethodName + ":exit", ClassName);
                LOG.Trace("InvokeScriptInternal: Method finishing: {0} ms from start", watch.ElapsedMilliseconds);
                return results;
            }
        }
    ...
    internal class MyRunspacePool
    {
        ...
        internal Runspace acquireRunspace() {
            ...
            if (_usedRunspaces.ContainsKey(id)) {
                LOG.Trace("Using unreturned runspace for thread {0}", id);
                ...
                if (_freeRunspaces.Count > 0) {
                    ...
                    LOG.Trace("No runspace available, creating new one.");
        ...
        internal void Close() {
            foreach (KeyValuePair<int,Runspace> p in _usedRunspaces) {
                LOG.Trace("Disposing of unreturned runspace (thread {0})", p.Key);
                p.Value.Dispose();
            }
